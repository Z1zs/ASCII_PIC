...
void FastPrinter::initDrawer() {
	...
	SetWindowPos(hwnd, HWND_TOP, 0, 0, (int)cfi.dwFontSize.X * (int)sizeX, (int)cfi.dwFontSize.Y * (int)sizeY, 0);
...
void FastPrinter::_draw() {
	...
	for (DWORD i = 0; i < sizeY; i++) {
		...
		WriteConsoleOutputCharacterA(hOutput, dataGrid + (i * (DWORD)sizeX), (DWORD)sizeX, coordBufCoord, &dWrite);
	}
	SetConsoleActiveScreenBuffer(hOutput);
}
